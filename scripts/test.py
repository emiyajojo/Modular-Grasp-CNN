...
def save_prediction_image(raw_pred, img_abs_path, img_root_path, im_size, out_dir):
    ...
    for i, (sem_pred, bbx_pred, cls_pred, obj_pred) in enumerate(zip(
        ...
        item = os.path.join(img_root_path[i], 'DATA','OCID_grasp',img_abs_path[i])
        im_size_ = im_size[i]
        ensure_dir(out_dir)
        ...
        # print("img_root_path[i]: %s, seq_path:%s" %(img_root_path[i], seq_path))
...
def make_config(args):
    ...
    conf = configparser.ConfigParser(allow_no_value=True, converters=_CONVERTERS)
    conf.read(args.config, encoding='utf-8')
